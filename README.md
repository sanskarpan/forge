...
![Forge real CLI execution capture](docs/assets/forge-pipeline.gif)

The demo asset is a reproducible recording of commands executed against this
checkout, generated by [`scripts/generate-demo-gif.sh`](scripts/generate-demo-gif.sh).
It is intentionally kept as a terminal capture so the documented pipeline is
grounded in observable Forge output rather than a simulated animation.
...
| Tooling | `eval`, `compile`, `asm`, `ir`, `cfg`, `regalloc`, `bench`, `verify`, `repl`, and `--verbose` phase timing |
| Workbench | Live editor, diagnostics, AST/IR/CFG, allocation and pressure, bytes, benchmarks, shareable URLs, targets |
...
cargo run -p forge-cli -- --verbose compile --emit 'x * x + 1'
...
To refresh the documented execution capture locally, install ImageMagick and
run `./scripts/generate-demo-gif.sh docs/assets/forge-pipeline.gif`.
