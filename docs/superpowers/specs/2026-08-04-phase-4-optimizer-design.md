...
- **Magic-number division** (Granlund-Montgomery) for `x / C` where `C` is a non-power-of-2 constant — the flagship optimization. PROMPT.md's `magic_signed`/`apply_magic` implementation (since corrected — an earlier draft's `apply_magic` was missing its `d` parameter, a real bug found while implementing this slice) is otherwise already correct and has its own exhaustive/property test (including `i64::MIN`) — port the math directly into `strength.rs`, don't reimplement from scratch. **Update, post-implementation:** the math is ported and tested standalone, but wiring it into an actual IR-rewriting `Pass` turned out not to be possible in this slice — our `Inst::Mul` is a 64-bit truncating multiply with no widening/128-bit variant, and the magic-multiply sequence fundamentally needs the high 64 bits of a 128-bit product. That instruction belongs in a later phase, once real x86-64 codegen exists to consult (`imul` producing a 128-bit result is a hardware primitive, not something to fake at the IR level). `strength.rs` documents this as a Phase 6/7 TODO; `magic_signed`/`apply_magic` are ready for that phase's instruction selection to call directly.
