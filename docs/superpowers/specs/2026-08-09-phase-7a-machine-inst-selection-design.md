...
2. `select(&Function) -> SelectedFunction` exists, walks blocks in RPO, and correctly lowers every `Inst` variant except `Call` (panics with a clear "ships in 7e" message) and `Rem` on `f64` operands (panics with a clear "float remainder not yet implemented" message; `Rem` on `i64` operands is fully implemented) into `MachineInst`s using virtual registers.
