...
1. **Write the invariant property tests before the implementation.** This design's own Testing section leads with the properties (no two overlapping intervals share a register, EXCEPT a legitimate same-instruction coalescing handoff — see Testing for the exact qualified statement; `active` stays sorted by end; every interval gets a `Location`; a real, non-trivial fraction of hints are actually honored), stated as corpus-wide assertions, not fixture-by-fixture pins — 8a's own example-based tests missed three separate forward-hint bugs across two review rounds; only a property test caught the whole class at once.
2. **Ask "can this even be satisfied?" as a design question, not an implementation afterthought.** Checked explicitly below for the hint-resolution rule — a hint is honored only against a register that is either genuinely free, or being handed off at exactly one point by its own hint target (never a blind override of something else) — 8a's design doc already flags that φ-groups routinely can't be fully co-located even at zero pressure.
...
**Deferred, explicitly out of scope, not silently missed**: `MachineInst::Shl`/`Shr`/`Sar`'s `rhs` operand has the exact same x86 hardware shape as `idiv`'s divisor conceptually (`shl`'s shift-amount operand must be in `Cl`/`Rcx`) — but `excluded_registers()` (8a) doesn't cover it, and this design doesn't add coverage for it either. This is DIFFERENT from `idiv`'s divisor case in one crucial way: a shift instruction doesn't DESTROY its `rhs` value the way `cqo`/`idiv` destroys the dividend's high bits — `rhs` can safely be relocated into `Cl` by an emission-time `mov` immediately before the shift, exactly like `idiv`'s `lhs`/dividend fixup (already resolved this way in 8a's design). The emitter's job is slightly bigger than just "move `rhs` into `cl`", though: that `mov` clobbers whatever ELSE currently occupies `Rcx` — which could genuinely be the shift's own coalesced `dst`/`lhs` (nothing excludes `Rcx` from that pair), not just an unrelated third party — so the emission-time fixup must displace-and-restore that occupant too, the same shape as idiv's already-accepted sub-problem-1 precedent, just with one more potential occupant to consider. So this is sub-problem 1 of 8a's idiv-clobber resolution (a value sitting in a needed register gets displaced and restored by emission, not by the allocator), not sub-problem 2 (a value that's destroyed and can't be recovered) — correctly out of scope for 8b's allocation-time exclusion mechanism, but recorded here explicitly so it isn't silently lost between slices.
