...
- A φ-merge test: confirms a φ's interval and its incoming values' intervals get the correct merged min-start/max-end range, AND that every group member's `hint` points at the canonical anchor (the φ's own destination `Value`) — the merge alone is not sufficient (see the corrected Hints section); the mutual-hint chain is what a review pass added after discovering the merge-only version left the allocator with a contradictory-looking, un-hinted set of overlapping intervals.
