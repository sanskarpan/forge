...
`min` and `max` are lowered with Forge's exact NaN and signed-zero rules:
SSE2, AVX2, and NEON use compare/select plus bitwise sign handling, while
AVX-512F uses an exact lane helper because its native minimum/maximum
...
/// scalar ops become vector ops, loads become vector loads, and one typed body
/// is reused by the loop at each induction offset. Acyclic `if` expressions
/// become lane masks and predicated selects, preserving inactive-lane values
/// even when they are NaN or infinity.
///
...
11. **No memory leaks.** Every `ExecutableBuffer` is `munmap`ped. The existing `getrusage(RUSAGE_SELF).ru_maxrss` high-water-mark regression test stays flat across 10,000 allocate/free cycles (`crates/forge-mem/tests/no_leaks.rs`), and Linux CI now runs that test executable under Valgrind Memcheck with definite/indirect leak errors failing the job (`.github/workflows/ci.yml`). Miri cannot model raw `mmap`/`mprotect`/`sysconf` syscalls or a transmute-to-function-pointer call, so it remains an explicit non-goal for this crate (see `forge-mem`'s crate-level doc comment).
