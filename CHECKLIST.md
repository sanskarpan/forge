...
The remaining array notes above are deliberately narrower follow-ups: the
current grammar accepts one row loop with `column[index]` loads and unindexed
f64 broadcasts, not arbitrary pointer arithmetic or nested source loops;
unsupported packed operations still use the verified scalar element fallback.
