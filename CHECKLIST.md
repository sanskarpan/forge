...
current grammar accepts one row loop with `column[index]` loads, not arbitrary
pointer arithmetic or nested source loops; unsupported packed operations still
use the verified scalar element fallback.
