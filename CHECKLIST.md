...
- The AArch64 scalar live-range reuse boundary is implemented on branch
  ...
  now use inclusive liveness intervals to reuse dead D/X temporaries, while
  edge-crossing and phi-participating CFG values receive stable unique homes.
  Genuine pressure, calls, stack-backed parameters, and unsupported shapes
  retain their established spill/ABI fallbacks. The focused AArch64 suite
  covers straight-line reuse, stable phi homes, native structured CFG
  execution, callee-saved frame use, and simultaneous-live spill cases.
