...
function AssemblyPanel({ artifact }: { artifact: CompileArtifact | null }) {
  ...
  return <div><div className="artifact-badge">encoding: {artifact.encoding ?? 'target unavailable'} · {artifact.bytes_len ?? artifact.wasm_bytes_len ?? bytes.length} bytes</div>
    {artifact.asm?.length ? <CodeBlock>{artifact.asm.map((item) => `${String(item.offset ?? 0).padStart(4, '0')}  ${(item.bytes ?? '').padEnd(24)}  ${item.text ?? ''}${artifact.encoding === 'wasm-stack' ? `  [stack ${item.stack_before ?? 0} → ${item.stack_after ?? 0}]` : ''}`).join('\n')}</CodeBlock> : <div className="assembly-empty">No target instruction artifact is available. Raw module bytes are shown below.</div>}
